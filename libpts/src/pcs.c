...
pcs_public_key* pcs_init_public_key(void)
{
    ...
    if (!pk) return NULL;
    mpz_inits(pk->g, pk->n, pk->n2, pk->maxInt, NULL);
    return pk;
}
...
pcs_private_key* pcs_init_private_key(void)
{
    ...
    mpz_inits(vk->p, vk->p2, vk->q, vk->q2, vk->hp, vk->hq, vk->mu,
              vk->lambda, vk->n, vk->n2, vk->maxInt, NULL);
    return vk;
}
                           ...
                           hcs_random *hr, const unsigned long bits)
{
    ...
    mpz_set(pk->n, vk->n);
    mpz_set(pk->n2, vk->n2);
    mpz_tdiv_q_ui(pk->maxInt, pk->n, 3);
    mpz_sub_ui(pk->maxInt, pk->maxInt, 1);
    mpz_set(vk->maxInt, pk->maxInt);
...
#else
    mpz_invert(vk->mu, vk->lambda, vk->n);      // what is mu?
...
void pcs_encrypt(pcs_public_key *pk, hcs_random *hr, mpz_t rop, mpz_t plain1)
{
    ...
    #pragma omp parallel sections
    {
        ...
        #pragma omp section
        {
            mpz_sub(t2, pk->n, pk->maxInt);
            if (mpz_cmp(plain1, t2) >0 && mpz_cmp(plain1, pk->n)<0){
                mpz_sub(plain1, pk->n, plain1);
                mpz_mul(plain1, pk->n, plain1);
                mpz_add_ui(plain1, plain1, 1);
                mpz_mod(plain1, plain1, pk->n2);
                mpz_invert(rop, plain1, pk->n2);
            }
            else{
                mpz_powm(rop, pk->g, plain1, pk->n2);
            }
...
void pcs_decrypt(pcs_private_key *vk, mpz_t rop, mpz_t cipher1)
{
    ...
    mpz_sub(t3, vk->n, vk->maxInt);
    if (mpz_cmp(vk->maxInt, rop) >=0){
	mpz_set(rop, rop);
    }
    else if (mpz_cmp(rop, t3) >0){
        mpz_sub(rop, rop, vk->n);
    }
    else{
	printf("Overflow detected in decrypted number");
	exit(0);
    }
    mpz_clear(t1);
    mpz_clear(t2);
